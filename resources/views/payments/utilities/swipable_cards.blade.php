@php
    ...
    $cardsData = $payments->map(function ($payment) use ($today, $urgentDate) {
        ...
        return [
            ...
            'po_id'          => $purchaseOrder?->folio ?? '—',
            'po_project'     => $purchaseOrder?->project ?? null,
            'project_name'   => $purchaseOrder?->projectRelation?->name ?? null,
            'work_name'      => $purchaseOrder?->workRelation?->name ?? null,
            'milestone_type' => $milestone?->type === 'anticipo' ? 'Anticipo' : 'Pago Regular',
            'due_date'       => $dueDate?->format('d/m/Y') ?? null,
            'due_date_long'  => $dueDate?->locale('es')->isoFormat('D [de] MMMM [de] YYYY') ?? null,
            'urgency'        => $urgency,
