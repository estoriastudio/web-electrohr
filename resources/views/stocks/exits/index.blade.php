...
document.addEventListener('DOMContentLoaded', function () {
	...
	function enableSection(inputs, enabled) {
		inputs.forEach(function (input) {
			...
			input.required = Boolean(input.name) && enabled;
		});
	}
	...
	modal.addEventListener('shown.bs.modal', function () {
		if (!conceptChoices && document.getElementById('concept_code')) conceptChoices = initializeRemoteSearch(document.getElementById('concept_code'), function (query) { return @json(route('concepts.search')) + '?q=' + encodeURIComponent(query) + '&limit=30'; }, 'Buscar código o descripción...', function (concept) { return concept.code + ' - ' + concept.description; }, function (concept) { return concept.code; });
...
<script src="{{ asset('assets/js/stock_exit_items.js') }}"></script>
@endpush
