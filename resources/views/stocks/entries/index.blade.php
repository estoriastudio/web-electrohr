...
<div class="card"><div class="card-header d-flex justify-content-between"><h4 class="card-title mb-0">Entradas</h4><button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#entryModal"><i class="ri-add-line me-1"></i>Nueva entrada</button></div><div class="card-body p-0"><div class="table-responsive"><table class="table align-middle text-nowrap table-hover table-centered mb-0"><thead class="bg-light-subtle"><tr><th>Fecha</th><th>Código</th><th>Tipo</th><th>Cantidad</th><th>Referencia</th></tr></thead><tbody>@forelse($entries as $entry)<tr><td>{{ $entry->received_at->format('d/m/Y') }}</td><td>{{ $entry->concept?->code ?? $entry->tool?->economic_number }}</td><td>{{ $entry->entry_type }}</td><td>{{ $entry->quantity }}</td><td>{{ $entry->purchase_reference }}</td></tr>@empty<tr><td colspan="5" class="text-center text-muted py-4">Sin entradas.</td></tr>@endforelse</tbody></table></div></div></div>
