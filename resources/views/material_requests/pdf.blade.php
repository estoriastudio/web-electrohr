@php
    ...
    $fechaNecesidad = $mr->need_date?->format('d/m/Y') ?? '—';
    $categoria      = $mr->supply_category ?? '—';
    $items          = $mr->items;
    $elaboradaPor   = $mr->requestedBy?->name ?? '—';
    $obsArray       = is_array($mr->observations) ? $mr->observations : [];
    ...
    <tr>
        <td style="padding: 0;">
            <table width="100%" cellpadding="5" cellspacing="0" border="1" style="border-collapse: collapse; border-color: #000; margin-top: -1px;">
                <tr>
                    <td style="border: 1px solid #000; padding: 5px 8px;">
                        <strong>Categoría de Suministros:</strong> {{ $categoria }}
                    </td>
                </tr>
            </table>
        </td>
    ...
    <tr>
        <td style="padding: 0;">
            <table width="100%" cellpadding="5" cellspacing="0" border="1" style="border-collapse: collapse; border-color: #000; margin-top: -1px;">
                ...
                <!-- Filas de materiales (loop Laravel) -->
