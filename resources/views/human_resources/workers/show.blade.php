...
@php
    ...
    $incentiveLabels = ['overtime' => 'Tiempo extra', 'day_off_exchange' => 'Libranza', 'emergency' => 'Emergencia'];
@endphp
            ...
            <div class="list-group list-group-flush">
                @foreach($documents as $path => $document)
                    @php
                        ...
                        $expirationDate = ($document['worker_expiration'] ?? false) ? $worker->{$document['expiration']} : $workerFile?->{$document['expiration']};
...
<div class="card mt-3">
    <div class="card-header border-bottom d-flex flex-wrap justify-content-between align-items-center gap-2">
        ...
        @if($worker->status !== 'terminated')<button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#createWorkerVacationModal"><i class="ri-add-line me-1"></i>Nueva solicitud</button>@endif
    </div>
        ...
        <div class="table-responsive">
            <table class="table align-middle text-nowrap table-hover table-centered mb-0">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="5" class="text-center text-muted py-4"><i class="ri-inbox-line fs-24 d-block mb-1"></i>No hay solicitudes de vacaciones.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
@if($worker->status !== 'terminated')
    ...
    <div class="modal fade" id="createWorkerIncentiveModal" tabindex="-1" aria-labelledby="createWorkerIncentiveModalLabel" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><form method="POST" action="{{ route('human_resources.incentives.store') }}">@csrf<input type="hidden" name="worker_id" value="{{ $worker->id }}"><input type="hidden" name="return_to_worker" value="1"><div class="modal-header"><h5 class="modal-title" id="createWorkerIncentiveModalLabel">Registrar incentivo</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="row g-3"><div class="col-md-7"><label class="form-label" for="workerIncentiveCategory">Concepto</label><select id="workerIncentiveCategory" class="form-select" name="category" required><option value="overtime">Tiempo extra</option><option value="day_off_exchange">Libranza</option><option value="emergency">Emergencia</option></select></div><div class="col-md-5"><label class="form-label" for="workerIncentiveRateType">Tipo</label><select id="workerIncentiveRateType" class="form-select" name="rate_type" required><option>A</option><option>B</option><option>C</option><option>D</option></select></div><div class="col-md-7"><label class="form-label" for="workerIncentiveDate">Fecha</label><input id="workerIncentiveDate" class="form-control" name="incentive_date" type="date" value="{{ old('incentive_date', now()->format('Y-m-d')) }}" required></div><div class="col-md-5"><label class="form-label" for="workerIncentiveStatus">Estatus</label><select id="workerIncentiveStatus" class="form-select" name="status" required><option value="active">Activo</option><option value="cancelled">Cancelado</option></select></div><div class="col-12"><label class="form-label" for="workerIncentiveNotes">Notas</label><textarea id="workerIncentiveNotes" class="form-control" name="notes" rows="3">{{ old('notes') }}</textarea></div></div></div><div class="modal-footer"><button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancelar</button><button type="submit" class="btn btn-primary">Guardar incentivo</button></div></form></div></div></div>
